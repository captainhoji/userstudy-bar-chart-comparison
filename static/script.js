...
async function initializeStudy(participantId, task) {
...
function loadInstructions(blockCounter, practice=true, easyPractice=false) {
  ...
  currentLayout = layout[blockCounter];
  currentOrientation = orientation[blockCounter];
  ...
  if (easyPractice) {
    ...
    if (task === "compare_height") {
      instructionText += `Each chart has one red bar. Your task is to <strong>identify which red bar is taller</strong>. `;
      ...
      instructionText += `Your task is to <strong>identify which red bar is longer</strong>. `
    }
    ...
    if (task === "compare_height") {
      ...
      // TODO: instructions of compare_length with examples
    }
  ...
  return function eventHandler(event) {
    if (event.code === "Space") {
      ...
      async function loadTrial() {
        ...
        answer = { compare_height: data[4], compare_index: data[3], compare_length: data[2] }[task]
...
// Set the flex direction based on layout (horizontal vs vertical)
  ...
  const chartSpec1 = drawBarChart('chart1', data[0], data[5][0], currentLabel);
  const chartSpec2 = drawBarChart('chart2', data[1], data[5][1], currentLabel);
...
function drawBarChart(elementId, values, redIndex, label) {
  // let darkestRed = Math.random() * 40 + 60
  let darkestRed = 100 - Math.floor(Math.random() * 2) * 100/7
  const data = values.map((value, index) => ({
    category: `${index+1}`,
    start: value[0],
    end: value[0] + value[1],
    color: index == redIndex ? darkestRed : Math.random() * (darkestRed-200/7)
  }));
  ...
  const chartSpec = {
    ...
    "data": {
      "values": data
      // "values": [
      //   {"category": "A", "start": 10, "end": 30, "val": 100}, ...
      // ]
    },
    "mark": "bar",
    "params": [
    ...
    "encoding": {
      [currentOrientation == "vertical" ? 'x' : 'y']: {
        "field": "category",
      ...
      [currentOrientation == "vertical" ? 'y' : 'x']: {
        "field": "start", 
        "type": "quantitative",
        "axis": {
          "title": false,
      ...
      [currentOrientation == "vertical" ? 'y2' : 'x2']: {"field": "end"},
      ...
      "color": {
        // "condition": {
        //   "test": "datum.category === highlightBar",
        //   "value": "rgba(255, 0, 0, 0.8)",  // 80% red
        // },
        "field": "color",  // or some numerical field
        "type": "quantitative",
        "scale": {
          "domain": [0, 100],  // replace with actual values
          //"range": ["rgba(254,224,210,1)","rgba(252,187,161,1)","rgba(252,146,114,1)","rgba(251,106,74,1)","rgba(239,59,44,1)","rgba(203,24,29,1)","rgba(165,15,21,1)","rgba(103,0,13,1)"]
          "range": ["rgba(254,240,217,1)","rgba(253,212,158,1)","rgba(253,187,132,1)","rgba(252,141,89,1)","rgba(239,101,72,1)","rgba(215,48,31,1)","rgba(153,0,0,1)"]
        },
        "legend": null
      }
    },
...
async function saveResponse(participantId, response) {
  ...
  if (isEasyPractice) {
    ...
    let explanation = ""
    const answer = { compare_height: data[4], compare_index: data[3], compare_length: data[2] }[task]
  ...
  if (!isPractice) {
    ...
    fetch('/save_response', {
      ...
      body: JSON.stringify({ 
        ...
        label: currentLabel,
        stimuli_number: numbers[blockCounter][trialCounter],
        duration: duration
      })
    });
  }
