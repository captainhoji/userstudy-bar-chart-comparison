...
# Accuracy plots use only non-"none" responses.
accuracy_data <- heatmap %>%
  filter(!response_time < 500) %>%
  filter(participant_id %in% eligible_ids, response != "none")
...
unanswered_by_lightness_label_pid <- heatmap %>%
  ...
  group_by(participant_id, lightness_mapping, label_condition) %>%
...
unanswered_by_lightness_label <- unanswered_by_lightness_label_pid %>%
  group_by(lightness_mapping, label_condition) %>%
  summarise(
...
message("Accuracy figures use only rows where response != 'none'.")
