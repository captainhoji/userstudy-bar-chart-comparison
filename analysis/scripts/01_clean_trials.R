...
heatmap <- readr::read_csv(cfg$raw_trial_heatmap, show_col_types = FALSE) %>%
  ...
  filter(!is.na(participant_id), !is.na(correct), !is.na(response_time), response_time >= 0)
...
participant_summary <- heatmap %>%
  ...
  mutate(
    ...
    tier1_exclude_low_heatmap_accuracy = heatmap_accuracy < tier1_accuracy_threshold,
